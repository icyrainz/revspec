...
Deferred — to be addressed when building the `/review-spectral` skill.
...
|---|---|
| tmux session (`$TMUX` set) | `tmux split-window -v "spectral <file>"` + `tmux wait-for` — seamless, human reviews in a pane |
| No tmux | Claude Code prints "Please run: `spectral <file>`" and polls for `.review.json` changes |
...
- Should we register a Claude Code hook that auto-launches spectral when a review file is created?
